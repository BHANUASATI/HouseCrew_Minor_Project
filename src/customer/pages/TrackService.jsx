...
export default function TrackService() {
  ...
  const fetchRequestDetails = async (requestId) => {
    try {
      // Find the request from the existing serviceRequests array
      const request = serviceRequests.find(req => req.id === requestId || req.id === parseInt(requestId));
      
      if (!request) {
        console.log('Request not found in serviceRequests:', requestId);
        throw new Error('Service request not found');
      }
      
      console.log('Found request in serviceRequests:', request);
      
      // Use the existing request data and enhance it with progress steps
