...
import { FaBars, FaUser, FaSun, FaMoon, FaChevronDown, FaSignOutAlt, FaCog, FaBell } from "react-icons/fa";
  ...
  return (
    <header className={`sticky top-0 z-50 backdrop-blur-xl border-b transition-all duration-300 ${
      ...
      <div className="flex items-center justify-between px-2 sm:px-2 lg:px-8 py-3 sm:py-2 lg:py-4 min-h-[60px] sm:min-h-[60px] lg:min-h-[80px]">
        ...
        <div className="flex items-center gap-1 sm:gap-2 lg:gap-4">
          ...
          <div className="flex items-center gap-1">
            <div className="hidden sm:block">
              <h1 className="text-lg sm:text-lg lg:text-xl xl:text-2xl font-bold">
        ...
        {/* RIGHT: DATE/TIME + NOTIFICATIONS + THEME TOGGLE + USER */}
        <div className="flex items-center gap-1 sm:gap-1 lg:gap-4">
          ...
          <div className="relative">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className={`p-2 sm:p-1.5 lg:p-2 rounded-lg transition-colors relative ${
                darkMode 
                  ? 'hover:bg-gray-700 text-white' 
                  : 'hover:bg-gray-100 text-gray-800'
              }`}
            >
              <FaBell className="text-lg sm:text-base lg:text-xl" />
              {unreadCount > 0 && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
