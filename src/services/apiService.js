...
class ApiService {
  ...
  static async getRazorpayConfig() {
    try {
      const response = await fetch(`${API_BASE_URL}/payments/razorpay-config`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error(data.detail || 'Failed to get Razorpay config');
      }
  ...
  static async getCustomerBills(customerId) {
    try {
  ...
  static async viewBill(billNumber) {
    try {
      const url = `${API_BASE_URL}/bills/${billNumber}/html`;
  ...
  static async downloadBill(billNumber) {
    try {
      const response = await fetch(`${API_BASE_URL}/bills/${billNumber}/download`);
  ...
  // QR Code Payment
  static async generatePaymentQR(amount, orderId, customerName = '') {
    try {
